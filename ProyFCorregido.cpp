...
//---------------------------------------------------------------------------------------------------------
//Procesar top 3 ciudades por pais
void procesarTop3Ciudades(const ListaDoble<Venta>& ventas) {
    ...
    for (int i = 0; i < ventas.getTamanio(); i++) {
        Venta v = ventas.getDato(i);
        string pais = v.getpais(), ciudad = v.getciudad();
        float monto = v.getMontoTotal();
        normalizar(pais); normalizar(ciudad);
        ...
        HashMapList<string, float>* mapa = ventas_ciudad_pais.get(pais);
        Lista<string>& lista_ciudades = ciudadesPorPais.get(pais);
        if (mapa->contieneClave(ciudad)) {
            mapa->put(ciudad, mapa->get(ciudad) + monto);
        } else {
            mapa->put(ciudad, monto);
...
//---------------------------------------------------------------------------------------------
//Procesar monto por producto de cada pais
void procesarMontoPorProductoYPais(const ListaDoble<Venta>& ventas) {
    HashMapList<string, HashMapList<string, float>*> mapa(50, funcionHashString);
    HashMapList<string, Lista<string>> productos(50, funcionHashString);
    Lista<string> paises;

    for (int i = 0; i < ventas.getTamanio(); i++) {
        Venta v = ventas.getDato(i);
        string pais = v.getpais(), prod = v.getproducto();
        float monto = v.getMontoTotal();
        normalizar(pais); normalizar(prod);

        if (!mapa.contieneClave(pais)) {
            mapa.put(pais, new HashMapList<string, float>(50, funcionHashString));
            productos.put(pais, Lista<string>());
            paises.insertarUltimo(pais);
        }

        HashMapList<string, float>* mapaProd = mapa.get(pais);
        Lista<string>& listaProd = productos.get(pais);
        if (mapaProd->contieneClave(prod)) {
            mapaProd->put(prod, mapaProd->get(prod) + monto);
        } else {
            mapaProd->put(prod, monto);
            listaProd.insertarUltimo(prod);
        }
    }

    mostrar_monto_por_pais_producto(mapa, paises, productos);
}

//-----------------------------------------------------------------------------------------------------------------------
//procesar promedio por categoria y pais
void procesarPromedioPorCategoria(const ListaDoble<Venta>& ventas) {
    Lista<pais_categorias> paises;
    agrupar_ventas_por_pais_categoria(const_cast<ListaDoble<Venta>&>(ventas), paises);
    mostrar_promedio_por_categoria(paises);
}

//----------------------------------------------------------------------------------------------------
//procesar medio de envio mas usado por pais
void procesarMedioEnvioPorPais(const ListaDoble<Venta>& ventas) {
...
//---------------------------------------------------------------------------------------------------------------
//procesar medio de envio mas usado por categoria
void procesarMedioEnvioPorCategoria(const ListaDoble<Venta>& ventas) {
...
//-------------------------------------------------------------------------------
//procesar dia con mas ventas
void procesarDiaConMasVentas(const ListaDoble<Venta>& ventas) {
    HashMapList<string, float> ventasPorDia(100, funcionHashString);
...
//----------------------------------------------------------------
//procesar estado de envio mas frecuente por pais
void procesarEstadoEnvioFrecuentePorPais(const ListaDoble<Venta>& ventas) {
    HashMapList<string, HashMapList<string, int>*> estadosPorPais(50, funcionHashString);
    HashMapList<string, Lista<string>*> estadosPorPais_claves(50, funcionHashString);
    Lista<string>* paises = new Lista<string>();

    for (int i = 0; i < ventas.getTamanio(); i++) {
        Venta v = ventas.getDato(i);
        acumularEstadosPorPais(v, estadosPorPais, estadosPorPais_claves, paises);
    }

    mostrarEstadoMasFrecuentePorPais(estadosPorPais, estadosPorPais_claves, paises);

    delete paises;
}

//procesar producto mas y menos vendido
void procesarProductosMasYMenosVendidos(const ListaDoble<Venta>& ventas) {
    HashMapList<string, int> cantidadPorProducto(50, funcionHashString);
    Lista<string> productos;

    for (int i = 0; i < ventas.getTamanio(); i++) {
        Venta v = ventas.getDato(i);
        acumularCantidadPorProducto(v, &cantidadPorProducto, &productos);
    }

    mostrarProductoMasYMenosVendido(&cantidadPorProducto, &productos);
    mostrarRankingProductos(&cantidadPorProducto, &productos);
}
...
void agregarVenta(ListaDoble<Venta>& ventas) {
    try {
        ...
        for (int i = 0; i < ventas.getTamanio(); i++) {
            if (ventas.getDato(i).getIDventa() == id) {
                throw runtime_error("El ID ingresado ya existe. Debe ser unico.");
            }
        }
...
void menuProcesamientoEstadisticas(ListaDoble<Venta>& ventas) {
    int opcion;

    do {
        cout << "\n=== Menu de Procesamiento de Estadisticas ===\n";
        cout << "1. Top 3 ciudades con mas ventas por pais\n";
        cout << "2. Monto total vendido por producto y pais\n";
        cout << "3. Promedio de ventas por categoria y pais\n";
        cout << "4. Medio de envio mas usado por pais\n";
        cout << "5. Medio de envío mas usado por categoria\n";
        cout << "6. Dia con mas ventas (por monto)\n";
        cout << "7. Estado de envio mas frecuente por pais\n";
        cout << "8. Producto mas y menos vendido por cantidad\n";
        cout << "9. Procesar todo\n";
        ...
        switch (opcion) {
            case 1: procesarTop3Ciudades(ventas); break;
            case 2: procesarMontoPorProductoYPais(ventas); break;
            case 3: procesarPromedioPorCategoria(ventas); break;
            case 4: procesarMedioEnvioPorPais(ventas); break;
            case 5: procesarMedioEnvioPorCategoria(ventas); break;
            case 6: procesarDiaConMasVentas(ventas); break;
            case 7: procesarEstadoEnvioFrecuentePorPais(ventas); break;
            case 8: procesarProductosMasYMenosVendidos(ventas); break;
            case 9:
                procesarTodo(ventas);
                break;
            case 0:
...
void start(ListaDoble<Venta>& ventas) {
    ...
    do {
        ...
        cout << "11. Deshacer ultima eliminacion"<<endl;
        cout<< "12. Procesar estadisticas."<<endl;
        cout << "0. Salir" << endl;
        ...
        switch (opcion) {
            ...
            menuProcesamientoEstadisticas(ventas);
            break;
            case 0:
